...
import { COMPANY_NAME, NAV_LINKS } from '../constants.tsx';
import { NavLink } from '../types.ts';
import Button from './common/Button.tsx';
import { BRAND_MASCOT_SRC } from '../assets.ts';
import AnimatedHamburgerIcon from './common/AnimatedHamburgerIcon.tsx'; // Import the new animated icon
...
const Navbar: React.FC = () => {
  ...
  useEffect(() => {
    const runAnimationCycle = () => {
      // 1. Move out
      const targetX = Math.floor(Math.random() * 7) + 2; // 2px to 8px (original value: * 220, too much)
      ...
      animationTimeoutRef.current = window.setTimeout(() => {
        // setCurrentAnimationDurationMs(moveDuration); // Animation duration for return is already set
        setDotTranslateX(0);
    ...
    // Initial call after a random delay
